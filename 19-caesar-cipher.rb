...
def caesar_cipher(offset, string)
  ...
  if offset > 0 # increment for positive offset
    string.each_char do |char|
      offset.times do
        # If it's not a z, we can just march forward
        if char.match(/[a-y]/)
          char.next!
        else # It's a z, account for wrapping
          char = 'a' if char.match(/z/)
        end
      end
      cipher << char # Add scrambled letter to the output message
    end
  else # decrement for negative offset
    string.each_char do |char|
      offset.abs.times do # absolute value, can't loop negative times!
        if char.match(/[b-z]/)
          char = (char.ord - 1).chr
        else # Reverse wrapping
          char = 'z' if char.match(/a/)
        end
      end
      cipher << char
    end
